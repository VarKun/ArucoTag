...
├── ArUco_detect2.py          # Variant with additional inline documentation (Chinese)
...
The script will detect markers in each photo, assemble 3D–2D correspondences, and save intrinsic parameters to `ArucoCheckMultiMatrix.npz` (camera matrix, distortion coefficients, and pose vectors).  `ArUco_detect.py` and `ArUco_detect2.py` load this file to undistort and estimate gate geometry.

### 5. Detect markers in real time

Choose the detector that fits your needs:

- `ArUco_detect.py` – Primary pipeline.  Highlights markers, detects the gate formed by IDs 0 and 1, reports gate area, and converts the horizontal midpoint to an alphabet character (lowercase by default, uppercase when the perceived gate size halves).
- `ArUco_detect2.py` – Same behaviour with inline Chinese documentation.
